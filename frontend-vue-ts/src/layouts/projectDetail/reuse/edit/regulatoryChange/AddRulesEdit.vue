...
const openEdit = async () => {
  ...
  if (regulatoryChanges && regulatoryChanges.add_rules) {
    // 深拷贝数据
    editingRules.value = JSON.parse(JSON.stringify(regulatoryChanges.add_rules));
  } else {
    // 确保editingRules始终是数组
    editingRules.value = [];
  }
...
const saveChanges = async () => {
  try {
    ...
    await reuseStore.updateRegulatoryChanges(projectId, {
      delete_rules: regulatoryChanges.delete_rules || [],
      add_rules: editingRules.value
    });
    ElMessage.success('新增规则已保存');
    dialogVisible.value = false;
  } catch (error) {
    console.error('保存新增规则失败:', error);
