  ...
  <div class="project-detail-page">
    ...
    <div class="page-header">
      ...
      <div class="page-actions">
        <StartRunButton :project-id="projectId" :stage="currentStage" page-type="effect-aware-reuse" />
        <el-dropdown>
          ...
          <template #dropdown>
            <el-dropdown-menu>
              ...
              <el-dropdown-item @click="handleDownloadIntermediate(4)">
                下载监管变更识别结果
              </el-dropdown-item>
              <el-dropdown-item @click="handleDownloadIntermediate(5)">
                下载级联影响范围分析结果
              </el-dropdown-item>
              <el-dropdown-item @click="handleDownloadIntermediate(6)">
                下载测试套件重用与更新结果
              </el-dropdown-item>
            </el-dropdown-menu>
          </template>
    ...
    <div v-if="currentStage === 0" class="document-detail-layout">
      ...
      <div class="right-panel">
        <IndependentRequirementResult :is-new-result="false" />
      </div>
    </div>
    ...
    <div v-else-if="currentStage === 1" class="document-detail-layout">
      ...
      <!-- 右侧结果区域 -->
      <div class="right-panel">
        <IndependentRequirementResult :is-new-result="true" />
      </div>
    </div>
    
    <div v-else-if="currentStage === 2" class="document-detail-layout">
      <!-- 左侧独立需求生成结果（旧文档） -->
      <div class="left-panel">
        <IndependentRequirementResult :is-new-result="false" />
      </div>
      ...
      <!-- 右侧测试场景合成结果（旧文档） -->
      <div class="right-panel">
        <TestScenarioSynthesisResult :is-new-result="false" />
      </div>
    </div>
    
    <div v-else-if="currentStage === 3" class="document-detail-layout">
      <!-- 左侧独立需求生成结果（新文档） -->
      <div class="left-panel">
        <IndependentRequirementResult :is-new-result="true" />
      </div>
      ...
      <!-- 右侧测试场景合成结果（新文档） -->
      <div class="right-panel">
        <TestScenarioSynthesisResult :is-new-result="true" />
      </div>
    </div>
    
    <div v-else-if="currentStage === 4" class="single-panel-layout">
      <!-- 结果区域 -->
    ...
    <div v-else-if="currentStage === 5" class="document-detail-layout">
      <!-- 左侧上下布局 -->
      <div class="left-panel">
        <!-- 左侧上部：场景-测试用例对齐关系图 -->
    ...
    <div v-else-if="currentStage === 6" class="document-detail-layout">
      <!-- 左侧上一步结果区域 -->
      <div class="left-panel">
        <CascadingImpactScopeAnalysisResult />
      </div>
...
// 阶段配置
const stages = [
  { id: 'independent-requirement-1', name: '独立需求生成 1' },
  { id: 'independent-requirement-2', name: '独立需求生成 2' },
  { id: 'test-scenario-synthesis-old', name: '测试场景合成（旧文档）' },
  { id: 'test-scenario-synthesis-new', name: '测试场景合成（新文档）' },
  { id: 'regulatory-change', name: '监管变更识别' },
  { id: 'cascading-impact', name: '级联影响范围分析' },
  { id: 'test-suite-update', name: '测试套件重用与更新' }
]
